...
int lowerBound(int num_step, State & state) {
    return state.lower_bounds[state.path[num_step]];
}
void curr_branch(vector<vector <int>> & ans, vector <vector <int>> grapth, int curr, State curr_state, State lower_state, int cst) {
    ...
    if(curr_state.path.size() == grapth.size()) {
        if(grapth[curr][curr_state.vertex] != INF &&
        grapth[curr][curr_state.vertex] + cst < lowerBound((int)curr_state.path.size(), lower_state)) {
